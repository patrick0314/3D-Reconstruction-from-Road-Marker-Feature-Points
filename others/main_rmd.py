...
from VisualUtils import ShortTimeReconstruct
import config
...
while True:
    ...
    frames_to_reconstruct = raw_image_front[center-7:center+7+1]
    ...
    for i in range(1+2*overlapping):
        ...
        # use less or more?
        # kp_road_center_more_matching = np.float32([kp_road_center[m.queryIdx].pt for m in matches_kp_road_more])
        # kp_road_curr_more_matching = np.float32([kp_road_curr[m.trainIdx].pt for m in matches_kp_road_more])
        
        if kp_road_center_more_matching.size == 0:
            print("No matching point!!", curr_idx)
            continue
        pcd_tmp = Front_STR.reconstruct(
            kp_road_center_more_matching,
            kp_road_curr_more_matching,
            proj_center,
            proj_curr,
            )
